...
def get_images():
    ...
    x_train = df_train.drop('label', axis=1).to_numpy(dtype=np.uint8)
    y_train = df_train['label'].to_numpy(dtype=np.uint8)
    x_test = df_test.drop('label', axis=1).to_numpy(dtype=np.uint8)
    y_test = df_test['label'].to_numpy(dtype=np.uint8)
